## 🥥 Game prototype available on Itch.io

https://linxit-studios.itch.io/fremdar

# 🚀 Fremdar — Godot.NET Prototype

A short first-person exploration game prototype built with Godot (.NET C#) as a 1-week university project, focused on technical systems, architecture, and rapid development.

The project demonstrates physics-based movement, modular state machines, and procedural environment elements within a compact development timeframe.

## ✨ Features

- 🎮 Physics-based first-person controller  
- 🧠 Modular **state-machine architecture** (player vehicle + NPCs)  
- 🤖 3D NPCs with Mixamo animations  
- 🌌 Real-time skybox generated based on location  
- 🚧 Obstacle detection and scripted events  
- 🔊 Minimal sound design and audio cues  
- ⚡ Rapid prototype built in 1 week  

## 🏗️ Technical Overview

- **Engine:** Godot.NET (C#)
- **Movement:** Physics-driven controller with State Machines + Component-based logic
- **AI:** Simple behavior states with transitions
- **Graphics:** Custom Environment made in Blender with Minimal Post Processing
- **Assets:** External animated models + Dynamic skybox system

## 🎯 Project Goals

- Practice gameplay architecture in Godot with C#  
- Implement reusable state-machine systems  
- Build a playable prototype in a short timeframe  
- Explore physics-based vehicle movement
- Implement skybox shader

## ⏱️ Development Time

- **Total development time:** ~1 week  
- **Estimated gameplay duration:** 10–15 minutes  

## 📦 Build / Run

1. Install **Godot with .NET support**
2. Clone the repository
3. Open the project folder in Godot
4. Run the main scene

## 📚 Learning Focus

This project focuses on:

- Game architecture in Godot.NET with Nodes and Resources
- Physics movement systems
- State machines
- Rapid prototyping workflows

## 🧑‍💻 Author

Popescu Florin-Iulian
Mercone Călin-Ștefan
Team Linxit Studios

University project created for learning purposes.
